...
13. **Artwork via Airlock**: poster JPEG → `/Airlock/Media/Artwork/<AssetID>` + `artwork_cache_id` in plist item dict. **Album/series portrait poster** (show-detail header big-poster slot): emit `insert_album` op with deterministic album_pid (SHA256(show+season) → 17-digit Int) and an `artwork_cache_id` in its item sub-dict, plus a `kind: 8` (kAlbumKind_TVShow) at op-top-level; upload portrait JPEG to `/Airlock/Media/Artwork/<album_pid>`; add `album_pid` to each insert_track's item dict so tracks bind to the explicit album row instead of medialibraryd auto-creating one (which would orphan our album row + its artwork). Pair `insert_artist` op with `artwork_album_pid: <album_pid>` so artist-level views also resolve. Track-class artwork (per-asset) drives only the episode-row landscape thumbs.
