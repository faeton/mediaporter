#!/usr/bin/env python3
"""
ATC sync with corrected insert_track fields based on MediaLibrary.sqlitedb analysis:
  - location = filename only (not full path)
  - base_location = directory without leading /
  - media_type = 8192 (video bitmask)
  - media_kind = 1024 (Home Video)
  - in_my_library = 1

All operations go in the CIG-signed plist at /iTunes_Control/Sync/Media/
since that's the only path the device consumes.
"""
import asyncio
import ctypes
...
TRIAL = sys.argv[3] if len(sys.argv) > 3 else str(random.randint(1,999))
...
# ============================================================
# [2] AFC: Upload file + write corrected sync plist
# ============================================================
...
# Split into base_location (no leading /) and location (filename only)
base_loc = f'iTunes_Control/Music/{slot}'
loc_fname = fname
p(f'\n[2] AFC: asset_id={asset_id}')
p(f'  device_path={device_path}')
p(f'  base_location={base_loc}, location={loc_fname}')

# Single CIG-signed plist with insert_track + corrected fields
sync_plist = plistlib.dumps({
    ...
    'operations': [
        ...
        },
        {
            ...
            'pid': asset_id,
            'item': {
                'title': f'TinyTest{TRIAL}',
                'sort_name': f'TinyTest{TRIAL}',
                'total_time_ms': 125,
                'media_kind': 1024,          # Home Video (iTunes DB format)
                'media_type': 8192,          # Video bitmask
                'year': 2026,
                'genre': 'Home Video',
                'in_my_library': 1,
            ...
            'track_info': {
                'location': loc_fname,        # Filename only
                'base_location': base_loc,    # Dir without leading /
                'file_size': file_size,
            },
        },
    ]
...
p(f'  Plist content preview:')
p(sync_plist.decode()[:500])
...
async def afc():
    ...
    async with AfcService(ld) as a:
        ...
        # CIG-signed plist with insert_track
        try: await a.makedirs('/iTunes_Control/Sync/Media')
        except: pass
        plist_path = f'/iTunes_Control/Sync/Media/Sync_{new_anchor:08d}.plist'
        await a.set_file_contents(plist_path, sync_plist)
        if cig_bytes:
            await a.set_file_contents(plist_path + '.cig', cig_bytes)
