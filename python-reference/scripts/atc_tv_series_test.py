#!/usr/bin/env python3
"""
Test TV series fields in ATC sync plist.

Based on working atc_nodeps_sync.py but replaces is_movie with TV series fields:
  - is_tv_show: True
  - tv_show_name, season_number, episode_number, episode_sort_id

Goal: verify that device creates DB entry with correct TV series metadata
in the TV app (not just as a generic movie).
"""
...
# ============================================================
# Helpers (same as atc_nodeps_sync.py)
# ============================================================
...
class NativeAFC:
    ...
    def write_file(self, path, data):
        handle = c_long(0)
        rc = MD.AFCFileRefOpen(self.afc_conn, path.encode('utf-8'), 2, byref(handle))
        if rc != 0: raise RuntimeError(f'AFCFileRefOpen({path}) failed: {rc}')
        CHUNK = 1048576
        offset = 0
...
# ============================================================
# TV Series test parameters
# ============================================================
VIDEO = sys.argv[1] if len(sys.argv) > 1 else 'test_fixtures/output/Breaking.Bad.S01E01.m4v'
...
p(f'=== TV Series Sync Test ===')
p(f'  Show: {SHOW_NAME}')
p(f'  Season {SEASON}, Episode {EPISODE}: "{EPISODE_TITLE}"')
p(f'  File: {VIDEO} ({file_size} bytes)')
...
# [3] Build sync plist — TV SERIES FIELDS
...
p(f'\n[3] Building TV series plist: asset={asset_id}, path={device_path}')
...
sync_plist = plistlib.dumps({
    ...
    'operations': [
        ...
        {'operation': 'insert_track', 'pid': asset_id,
         'item': {
             'title': EPISODE_TITLE,
             'sort_name': EPISODE_TITLE.lower(),
             'total_time_ms': 5000,  # ~5s test file
             'date_created': now,
             'date_modified': now,
             # --- TV SERIES FIELDS (replacing is_movie) ---
             'is_tv_show': True,
             'tv_show_name': SHOW_NAME,
             'sort_tv_show_name': SHOW_NAME.lower(),
             'season_number': SEASON,
             'episode_number': EPISODE,
             'episode_sort_id': EPISODE,
             # --- Common fields ---
             'remember_bookmark': True,
             'artist': SHOW_NAME,
             'sort_artist': SHOW_NAME.lower(),
             'album': f'{SHOW_NAME}, Season {SEASON}',
             'sort_album': f'{SHOW_NAME.lower()}, season {SEASON}',
             'album_artist': SHOW_NAME,
             'sort_album_artist': SHOW_NAME.lower(),
         },
         'location': {'kind': 'MPEG-4 video file'},
         'video_info': {
             'has_alternate_audio': False, 'is_anamorphic': False,
             'has_subtitles': False, 'is_hd': True,  # 1080p
...
if not got_manifest:
    p('No AssetManifest — TV series fields may have been rejected.')
    p('Try checking if the plist was consumed (file should disappear from device).')
    sys.exit(1)

# [7] FileBegin + FileComplete
...
p(f'\n=== RESULTS ===')
p(f'Show: {SHOW_NAME}')
p(f'Episode: S{SEASON:02d}E{EPISODE:02d} "{EPISODE_TITLE}"')
p(f'Path: {device_path}')
p(f'')
p(f'CHECK: Open TV app on iPad:')
p(f'  1. Does the episode appear under "TV Shows" (not "Movies")?')
p(f'  2. Is it grouped under "{SHOW_NAME}"?')
p(f'  3. Does it show Season {SEASON}, Episode {EPISODE}?')
p(f'  4. Check MediaLibrary.sqlitedb for media_type and media_kind values.')
